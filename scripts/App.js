class App {
  ...
  async displayRecipes() {
    this.recipesList = recipes.map((recipe) =>
  ...
  toggleElementVisibility(
      ...
      filteredList.map((filter) => {
        const value =
          typeof filter === "object"
            ? filter[optionProperty].toLowerCase()
            : filter.toLowerCase();
        return value;
      })
    );
    ...
    $elementList.forEach(($element) => {
      const elementValue = useTextContent
        ? $element.textContent.toLowerCase()
        : $element.dataset.option.toLowerCase();
  ...
  updateOptionsList() {
    const selectedOptionsSet = new Set(
      this.selectedOptionsList.map((option) => option.toLowerCase())
    );
  ...
  handleMainSearchInput(optionProperty, $dropdown) {
    this.$mainSearchInput?.addEventListener("input", () => {
      ...
      this.$mainCrossIcon.classList[this.mainInputValue ? "remove" : "add"](
        "hidden"
      );
  ...
  filterRecipesBySearch() {
    ...
    for (let i = 0; i < this.recipesList.length; i++) {
      ...
      if (this.mainInputValue.length < 3) {
        matchesSearch = true;
      } else if (regex.test(recipe._name) || regex.test(recipe._description)) {
      ...
      for (let j = 0; j < this.selectedOptionsList.length; j++) {
        if (
          !recipe.ingredientsList.includes(this.selectedOptionsList[j]) &&
  ...
  handleSearchInput(
    ...
    $searchInput.addEventListener("input", () => {
      ...
      $allDropdown.forEach(($dropdown) => {
        const $currentOptionsButtons =
          $dropdown.querySelectorAll(".option-button");
        const $noOptionMessage = $dropdown.querySelector(".no-option-message");

        if (
          Array.from($currentOptionsButtons).every((button) =>
  ...
  filterOptionsBySearch(input) {
    ...
    const optionsList =
      this.updatedOptionsList.length > 0
        ? this.updatedOptionsList
        : this.optionsList;
    ...
    const currentSearchOptionsList = optionsList.filter((option) =>
      regex.test(option)
    );
  ...
  handleOptionButton($dropdown, optionProperty, $input) {
    ...
    $optionsButtons.forEach(($optionButton, index) => {
      $optionButton?.addEventListener("click", () =>
        handleOptionClick($optionButton, index)
      );
    });
  }
  ...
  filterRecipesByOption() {
    ...
    this.filteredRecipesList = this.recipesList.filter((recipe) => {
      const matchesSearch =
        this.mainInputValue.length < 3 ||
        inputRegex.test(recipe._name) ||
        inputRegex.test(recipe._description) ||
        recipe._ingredients.some((ingredient) =>
          inputRegex.test(ingredient.ingredient)
        );

      const matchesFilters = this.selectedOptionsList.every(
        (selectedOption) => {
          const optionRegex = new RegExp(selectedOption.trim(), "i");
          return (
            recipe.ingredientsList.some((ingredient) =>
              optionRegex.test(ingredient)
            ) ||
  ...
  appendOptionButtonSelected($dropdown, optionText) {
    const $buttonSelectedContainer = $dropdown.querySelector(
      ".button-selected-container"
    );
    const $optionSelected = dropdown.appendOption($dropdown, optionText);
    $optionSelected.classList.add(
  ...
  styleOptionButtonSelected($optionsButtons, index) {
    const $selectedButtonList = document.querySelectorAll(
      ".option-button-selected"
    );

    const allOptionsHidden = Array.from($optionsButtons).every(
      ($optionButton) => $optionButton.classList.contains("hidden")
    );
    ...
    $selectedButtonList.forEach(($button, index) => {
      ...
      const $closeButtonSelected = $button.querySelector(
        ".close-option-button"
      );
  ...
  handleCloseOption($dropdown, optionProperty) {
    const $closeButtonsSelected = document.querySelectorAll(
      ".close-option-button"
    );
    const $closeOptionsSelected = document.querySelectorAll(
      ".close-option-selected"
    );

    this.handleCloseOptionClick(
      $closeButtonsSelected,
      $dropdown,
      optionProperty
    );
    this.handleCloseOptionClick(
      $closeOptionsSelected,
      $dropdown,
      optionProperty
    );
  }
  ...
  async init() {
    ...
    const ingredientsElements = this.getDropdownElements(
      this.$ingredientsDropdown
    );
    const appliancesElements = this.getDropdownElements(
    ...
    this.handleSearchInput(
      this.$ingredientsDropdown,
      ingredientsElements.$searchInput,
      ingredientsElements.$crossButton,
      ingredientsElements.$crossIcon,
      ingredientsElements.$options,
      "ingredientsList"
    );
    this.handleSearchInput(
      this.$appliancesDropdown,
      appliancesElements.$searchInput,
      appliancesElements.$crossButton,
      appliancesElements.$crossIcon,
      appliancesElements.$options,
      "appliance"
    );
    this.handleSearchInput(
      this.$utensilsDropdown,
      utensilsElements.$searchInput,
      utensilsElements.$crossButton,
      utensilsElements.$crossIcon,
      utensilsElements.$options,
      "utensils"
    );
